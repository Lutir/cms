	...
	if($menutype=="classic")
	{
		...
		if($pageRow['page_displaysiblingmenu']) {
			if($pageId != 0) {
				...
				$menuHtml .= '<a href="'.$hostURL.'../"><h3>'.$imageTag.$parentPageRow["page_title"].'</h3></a>';
...
function getChildList($pageId,$depth,$rootUri,$userId,$curdepth) {
  ...
  $var = "<div class='div_{$classname}'><ul class='{$classname} depth{$curdepth} nav side-menu'>";
  for($i=0;$i<count($pageRow);$i+=1) {
	  ...
	  if($pageRow[$i][4]) {
	  	 ...
	  	 *@code:
	  	  else { href=\"".$rootUri.'/home'.getPagePath($pageRow[$i][0])."\"
	  		global $cmsFolder;
	  		global $templateFolder;
	  		$imageTag = "<img src=\"{$hostt}/$cmsFolder/$templateFolder/common/images/folder.png\" alt=\"{$pageRow[$i][1]}\" width=\"16\" height=\"16\"/>";
	  	}*/
	  }
	  $var .= "\n<li><a class=\"reset-this \" href=\"".$rootUri.'/home'.getPagePath($pageRow[$i][0])."\">".$imageTag." ".$pageRow[$i][2]."</a>";
